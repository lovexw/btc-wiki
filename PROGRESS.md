...
>
> 最后更新：2026-09-13（术语精翻 B7 第五批完成 50/50，累计 468/476；第六批收官 8 条从 `transaction-chaining` 起，断点详情见下方「B7 断点」）
...
| glossary/ 术语表（全文精翻） | 476 | 468 | ⏳ 进行中，按下方批次计划推进（B7 第一至五批已完成，仅剩第六批收官 8 条） |
...
- **B7 第五批（50 条）✅ 2026-09-13 完成**（`pay-ip` 至 `tracking-error`，全部清单见上方「已精翻」B7 段）。
...
- **第五批备注（2026-09-13）**：`security` 词条末尾链接[安全指南](/security)（`docs/security.md`，根路径、无尾斜杠，写成 `/security/` 会被 STRICT_LINKS 判死链 `/security/index`）；源文 `/rabbit-hole/` 单数链接一律改 `/rabbit-holes/` 复数；NIST csrc.nist.gov 外链按外链政策删除、正文表述保留（BIP GitHub 链接保留）。
...
| B7 | 字母序扫尾（第一至五批各 50 条 ✅ 2026-09-13 完成，断点清单见上方） | 不匹配上述关键词的其余全部剩余词条，每 50 条一批 | 108（已完成 250，剩 8：第六批收官） |
